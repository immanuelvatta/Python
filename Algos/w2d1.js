...
function makeFrequencyTable(arr) {
    // create a condition to check if the array is empty
    if (arr.length == 0){
        return {}
    }
    else{
        ...
        for (let value of arr){
            //conditionals
            // if there is not a key then you add the key and give an increment
            //if there is a key you increment it by 1
            if(result[value]){
                result[value] +=1;
            }else{
                result[value] = 1
